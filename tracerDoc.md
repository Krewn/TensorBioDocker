...
For more information on TraCeR, its validation and how it can be applied to investigate T cell populations during infection, see our paper in Nature Methods or the bioRxiv preprint that preceded it.
...
Software
    ...
    Bowtie2 - required for alignment of reads to synthetic TCR genomes.
    Trinity - required for assembly of reads into TCR contigs. TraCeR now works with both version 1 and version 2 of Trinity. It should automatically detect the version that is installed or you can specify it in the config file.
        Please note that Trinity requires a working installation of Bowtie v1.
    IgBLAST - required for analysis of assembled contigs. (ftp://ftp.ncbi.nih.gov/blast/executables/igblast/release/).
    Kallisto - required for quantification of TCR expression.
    Graphviz - Dot and Neato drawing programs required for visualisation of clonotype graphs.
...
Python modules
    ...
    Levenshtein
    Networkx
    It seems that v1.11 of Networkx behaves differently when writing dot files for use with Graphviz. If you have this (or later versions) you also need to install PyDotPlus.
...
This will peform the assemble step using the small test dataset. It will then perform summarise using the assemblies that are generated along with pre-calculated output for two other cells (in test_data/results).
...
The following output files are generated:
    ...
    recombinants.txt List of TCR identifiers, lengths and productivities for each cell.
    ...
    clonotype_network_[with|without]_identifiers.dot files describing the clonotype networks in the Graphviz DOT language
